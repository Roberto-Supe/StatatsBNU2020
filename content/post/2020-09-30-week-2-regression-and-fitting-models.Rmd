---
title: Week 2 Regression and Fitting Models
author: Roberto Supe
date: '2020-09-30'
...
This week we will cover data summaries, data structure, linear regression and multiple regression and their assumptions.

At the begining of each session we will need to load our libraries, so let's load the packages for this week
...
Mean, variance and standard error are allways required for the final report. It can take a while to sort the data in the right way and several times you may want to check how your data looks frequently. `Tydiverse` is a complex group of packages which unclude `ggplot2` and `dplyr`. Addiding, changing, combining, removing is simple with `dplyr`.
...
I am sure that you are familiar with that as it is something we can get from any statistical program, now, let's look how to deal with more information and different results.
...
* **summarise_all** means that all the functions will be apply to all the variables excluded from `group_by`.
* **.** and **na.rm**: the `.` means take all the information as it is no changes. `na.rm=T` means TRUE (yes) to remove missing values if there are some 

You want to know if there is a relationship between wind and pressure so we need to visuallize both variables in a scatter plot. 
...
# we will begin by ploting the variables of our hypothesis
ggplot(storms,mapping = aes(x=wind,y=pressure))+
  geom_point()
```

Looks like there is some sort of relationships, negative relationship, between our variables of interest. However, we need statistical models to determine the changes in presserue with wing and how significant are those changes.

# Least squares Linear Regression:

Least squares method also deffined as the *line of best fit* aims to produce a model which reduces the *variation *or *error* between the expected and predicted values. Next we will run a simple linear regression with `lm` with our two variables to understand this concept and the estimation of model parameters which are often called **ordinary least squares estimates**. The previous scatter looked roughly linear, so let's examine their relationship.
...
Therfore the equation for the LSLR with the CI at 95% confidence level will be: 1029.66 - 0.7015x ,phantom(.)%+-%phantom(.), &#177 6.537. 

## Confidence Intervals (CI)

They are values which also represent the accuracy of our sample mean. This will show a range of plausible values not only for the mean but for any unknown parameter.   
...
The model explains 88.78% of the variation in  pressure by wind data.

# Simple Linear Regression

The regression model was done and we needed to know how significant are those results. However, before we begin, it is important that you check the data before regression and any other model that you run. By this, it is ment that we should determine something like the normality of the data. There are multiple tests and if not normally distributed,  we will need to transform the data. 

## Assumption One (Normality)

Use `shapiro.test` to carry out the normality test, here, the H0 is that data is normally distributed. P-Values >0.05 will support this normal distribution of your variable. 
...
shapiro.test(iris$Sepal.Length) # normality test failed
shapiro.test(log(iris$Sepal.Length)) # normality test with transformed data passed

```

lrearly, we can make more sence of the data when we plot it. This will be useful when you got a lot of variables.
...
ggplot(data=iris,mapping = aes(sample=Sepal.Length))+
  ...
  geom_qq_line()+
  theme_bw()
#transformed data with an alternative package to ggplot2
ggpubr::ggqqplot(log(iris$Sepal.Length)) # set the variable that you want to get the qqplot
```

* Data transformation resulted in a better qq plot. Now we can see why it passed `shapiro.test`.

* We used two options, one plot with the normal ggplot2 and the second one with the package `ggpubr`. The last is a package that reduces ggplot2 lines of code and gives you great results. If you work with small data sets and you want to have good graphs it is a great option. We will cover more about this package in a few weeks.

Let's build a data set for our regressions 
...
## Assumption 2 (Lineariy)

The relationship between the independent and dependent variable must be linear. Scatter plot is our alternative to test if there is a linear distribution which would show our points is something like a straight line.
...
Something that will save us time is `chart.Correlation` with this function we get the histograms, scatter plots, correlations and the significance of those correlations. 
...
Now that all the variables are normally distributed and there is linear relationship we can use them in our regression regression. 
...
We can see that bacteria population did not change with temperature (P-value>0.05). Let's see how radiation affects bacteria population
...
We got similar results so we would conclude that we are 95% confident that radiation and temperature alone do not affect the population of bacterias. 95% confident because we run our model at this CI.

## Model test assumption  (Homoscedasticity)

We should ensure that the model fited meets this last assumption, same variance over the range of prediciton, to maintain the model. This means that the error term needs to be the same across all values of the independent variables. The fitted model already has all the imformation that we need for the plots so we will just call plot(model). 
...
Here we can see that the resudial erros are constant across the groups and distributed evenly. Look at the `Normal Q-Q` (top right) which under homoscedasticity it should show the points along the line.

* This will conclude a simple linear regression model

> When we got a lot of variables, We can do the test in one line of code with **lappy**. Additionally, when you run multiple variables the resulting object is a list (results of the first, results of the second, results of the n) to observe and export all those values or use them in other parts of your documents then we need to unlist and save them in a data frame. 

## Extra Notes 
### Run the same test for each variables

```{r}
results<-data.frame(do.call(rbind,lapply(df[,1:3],function(x) shapiro.test(x)[c("statistic","p.value")])))
...
There is no one option and you can also do something like this
...
Here there is a function to get the variable correlations, histograms and scatter plot with text over the histograms.
...
```{r, out.width="50%",fig.show='hold',error=FALSE}
library(qqplotr) #install.packages("qqplotr")
...
## Rearange multiple variables for ggplot

To use ggplot2 in a more advance level you need to set a long data format. This means that all your response variables need to be in rows and the grouping in a different variable.
...
That is a great and fast way to see distributions and you can many plots at once.
